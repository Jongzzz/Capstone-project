...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const requestData = {
        ...
        pain: Number(formData.pain_score),
        systolic: Number(formData.sbp),
        diastolic: Number(formData.dbp),
        temperature: Number(formData.temperature),
        heart_rate: Number(formData.heart_rate),
        resp_rate: Number(formData.resp_rate),
        spo2: Number(formData.o2sat),
  ...
  const getLevelStats = () => {
    ...
    return [
      { name: 'Lv.1 소생', value: counts[1], fill: '#DC2626' },
      { name: 'Lv.2 긴급', value: counts[2], fill: '#EA580C' },
      { name: 'Lv.3 응급', value: counts[3], fill: '#CA8A04' },
      { name: 'Lv.4 준응급', value: counts[4], fill: '#16A34A' },
            ...
            <div className="dashboard-card">
              ...
              <form onSubmit={handleSubmit} className="triage-form" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <div className="form-group">
                  ...
                  <div className="chief-complaint-selector">
                    <button type="button" className={`complaint-chip ${formData.gender === 1 ? 'active' : ''}`} onClick={() => setFormData({ ...formData, gender: 1 })}>👨 남성</button>
                ...
                <tbody>
                  {patientHistory.map((p, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #E2E8F0', opacity: p.isActive ? 1 : 0.6 }}>
                      ...
                      <td style={{ padding: '10px', fontWeight: 600 }}>{maskName(p.name)}</td>
                      <td style={{ padding: '10px' }}>{p.age}세</td>
                      <td style={{ padding: '10px' }}>{p.gender === 1 ? '남' : '여'}</td>
          ...
          <div className="dashboard-grid-3" style={{ gridTemplateColumns: '1fr 1fr' }}>
            <div className="dashboard-card">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie data={getLevelStats()} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={5} dataKey="value">
                    {getLevelStats().map((entry, index) => <Cell key={`cell-${index}`} fill={entry.fill} />)}
                  </Pie>
